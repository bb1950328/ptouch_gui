...
class Device(object):
    ...
    def render_preview(self, design: data.Design) -> pathlib.Path:
        png_path = pathlib.Path(tempfile.gettempdir()) / "ptouch_preview.png"
        command = design.get_command()
        try:
            command.initialize(self.get_info())
            out, err = self._run_command(command.get_command() + ["--writepng", str(png_path)])
        finally:
            command.cleanup()
        print(out, err)
    ...
    def print(self, design: data.Design, num_copies: int = 1) -> None:
        command = design.get_command()
        try:
            command.initialize(self.get_info())
            out, err = self._run_command(["--copies", str(num_copies)] + command.get_command())
        finally:
            command.cleanup()
        # todo check out and err
        print(out, err)
